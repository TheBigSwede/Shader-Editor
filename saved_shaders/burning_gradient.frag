...
/*     vec3 colorA = vec3(0.42f, 0.04f, 0.91f);
    vec3 colorB = vec3(0.99f, 1.0f, 0.58f); */
/*     colorA = rgb2hsv(colorA);
    colorB = rgb2hsv(colorB); */

    //float pct = floor(4.*(fragcoord.y+0.2*One_D_Perlin_Oct(fragcoord.x*5., 20)))+2.;
    //float pct = floor(4. * (fragcoord.y + 0.2 * TwoD_Perlin(fragcoord * 1.))) + 2.;
    float pct = floor(4. * (fragcoord.y + 0.05 * simplex_noise_oct((vec2(-0.1*u_time,-0.01*u_time)+fragcoord)*10.,5)));
